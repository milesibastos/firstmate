...
- Helper scripts in `bin/` are plain bash.
  ...
  They run with errexit off so each test can tolerate a nonzero exit and report it through `fail`, and a suite that turns errexit on is refused by name; `tests/lib.sh` owns why that matters and how to fix a suite that does.
...
- On `pull_request`, GitHub takes a workflow's definition from head merged into base, so a step added to `main` is live on every open pull request before any of them rebases.
  `actions/checkout` separately decides which tree that step's files come from, and its default ref is that same merge commit, so a repository script added alongside a new step arrives with it.
  Pinning `ref:` to the pull request head splits the two: the new step then runs against a tree that predates the script it calls, and every branch based before that commit fails with a bare exit 127 until it rebases.
  Take the default merge ref unless a step genuinely needs the head tree, and where it does, guard the call so the failure names its cause instead of exiting 127 anonymously.
