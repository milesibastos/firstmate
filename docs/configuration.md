...
FM_PROC_ROOT_OVERRIDE=   # alternate /proc root for Linux process reads (fm_pid_identity in fm-wake-lib.sh, the argv[0] probe in fm-cursor-lib.sh), mainly for tests
...
FM_SNAPSHOT_TASK_CONCURRENCY=8   # per-task current-state reads bin/fm-fleet-snapshot.sh keeps in flight at once; those reads are almost entirely spent blocked on one no-mistakes round trip each, so running them together is what keeps a snapshot's cost from growing with the fleet, and the cap is what stops a large fleet opening one client per task at once; invalid or zero values are refused
FM_SNAPSHOT_CANCEL_GRACE=2   # seconds an aborted bin/fm-fleet-snapshot.sh waits for the per-task reads it started to stop before stopping them hard; the command is read-only, so a caller that stopped waiting leaves nothing worth finishing; invalid or zero values are refused
...
FM_LOCK_MAX_STEAL_DEPTH=8   # ceiling on recursive ".steal" mutex escalation depth during a stale-owner reclaim; reaching it refuses the acquire instead of recursing further
...
FM_STALE_ESCALATE_SECS=240         # idle seconds before a provably-working stale pane escalates; a declared external wait or verified captain-held transfer takes the FM_PAUSE_RESURFACE_SECS recheck below instead even while its crew is provably working; stale panes whose crew is not provably working surface immediately unless they declare the pause verb
...
FM_WEDGE_DEMAND_INSPECT_COUNT=3    # consecutive provably-working stale escalations on the same unchanged pane before demand-deep-inspection is added; the same count of consecutive bounded re-surfaces of an absorbed pane, declared wait or worktree-write deferral alike, adds it to that recheck's reason
