...
Actionable wakes include captain-relevant status signals, no-verb signals without positive evidence that their crew is still executing, authenticated check output such as PR merge polling or a Relay mention, stale panes whose crew is not provably working whether their status log looks terminal or non-terminal, provably-working stale panes that persist past `FM_STALE_ESCALATE_SECS` without their own task worktree being written and without a declared wait standing, declared external waits and verified captain-held transfers that remain declared past `FM_PAUSE_RESURFACE_SECS`, and heartbeat backstop hits.
...
Consecutive bounded re-surfaces of the same absorbed pane, whether it was absorbed for a declared wait or for worktree writes, carry that same marker at that same threshold, so absorbing a pane bounds how long its evidence is taken on trust instead of exempting it.
...
A still-declared wait whose crew is authoritatively working takes that cadence too, because an actively running step is positive liveness proof and a crew blocked in a foreground pipeline call has exactly that shape: a running step and a frozen pane.
This is the same exception the busy-pane bound above already grants on a busy verdict, and the same one the away-mode daemon grants on the declaration alone, so one declaration means one thing across both supervisors and both pane states.
...
On that unread-surface-only path the cursor stops before any terminal captain-relevant line it did not print, so a `done:` or `failed:` is re-presented later rather than recorded as presented by a surface that never showed it.
`needs-decision` and `blocked` are exempt from that stop only when the same key and reserved-namespace checks the OPEN DECISIONS fold applies accept the line, because that fold re-surfaces such a line on every drain until it closes; a needs-decision/blocked line the fold itself skips has no such standing surface and halts the cursor like any other terminal event, whose one-shot presentation is the only one it gets.
...
Each of those markers dedupes only its own supervisor's paths, so the daemon additionally floors its scan position at the fleet-wide `state/.status-presentation-cursor`: a stretch in which firstmate supervised while the daemon was stopped is proof those bytes were already presented, and without that floor every such gap would replay as new at the next away-mode entry.
...
`bin/fm-control.sh` is the control plane: an allowlisted `interrupt`, `exit`, transactional `relaunch`, and endpoint-rebuilding `reconcile`, addressed to an exact task id, with per-harness mechanics owned by `bin/fm-control-lib.sh`, a verified postcondition per verb, and no arbitrary-text or raw-key entry point.
