...
`needs-decision` and `blocked` are exempt from that stop because the OPEN DECISIONS fold re-surfaces them on every drain until they close, while a terminal event's one-shot presentation is the only one it gets.
