...
Those markers and the fleet-wide presentation cursor are two records of ONE audience, the captain, and each only advances while its own side is supervising, so the daemon's marker goes stale across every stretch attended firstmate supervised while the daemon was stopped.
Away-mode entry therefore seeds the daemon's marker from the presentation cursor once per session, keyed to the identity of `state/.afk`, rather than consulting that cursor continuously: once away mode begins the only drains that run are the daemon's own, which present to nobody, so the cursor stops being evidence of anything the captain saw.
