...
`tests/fm-watcher-lock.test.sh` covers verified-successor attach, recovery publication before stale-lock removal, the typed self-eviction failure, bounded and successor-linked lifecycle rows, a SIGSTOP counterfactual that distinguishes a live PID from a stale beacon before classifying termination, the start-time identity proof that reclaims a recycled pid on this same lock without evicting a live holder whose command merely changed, the shared `fm_identity_holder_is_current` predicate's full verdict matrix separating not-disproved from actually-proved, and a real-exec case that records a process's identity while it is still bash and proves the match survives its exec rather than passing vacuously.
