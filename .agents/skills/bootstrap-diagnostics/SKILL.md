...
- `FLEET_PUBLISH: <reason>` - this home opted in to a snapshot cadence (`docs/configuration.md` "Fleet snapshot cadence") and session start could not get its publisher running, so the file-watching surface that depends on it is frozen at whatever `state/fleet-snapshot.json` last held.
  A configuration reason means the cadence file is unusable and is being refused rather than defaulted; correct the file, then rerun `bin/fm-fleet-publish.sh start`.
  An unconfirmed-start reason means the publisher was launched but never reported a live process with a fresh beacon; run `bin/fm-fleet-publish.sh status` and read `state/.fleet-publish.log` for the recorded refusals before starting it again.
  Arming is idempotent and never touches the published snapshot, so a retry cannot lose the existing picture.
